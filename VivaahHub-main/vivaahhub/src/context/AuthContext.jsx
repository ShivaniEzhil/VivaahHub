...
export const AuthProvider = ({ children }) => {
  ...
  const signIn = async (phone) => {
    ...
    if (!isValidIndianPhone(normalized)) {
      toast.error("Enter a valid 10-digit mobile number after +91 (e.g. 9876543210)", { toastId: "signInPhone" })
      return false
    }
    setLoading(true)
    try {
      await api.post("/users/sign-in", { phone: normalized })
      return true
    } catch (err) {
  ...
  const verifySignInOtp = async (phone, otp) => {
    ...
    setLoading(true)
    try {
      const res = await api.post("/users/verify-sign-in-otp", { phone: normalized, otp })
  ...
  const signUp = async (full_name, phone) => {
    ...
    if (!isValidIndianPhone(normalized)) {
      toast.error("Enter a valid 10-digit mobile number after +91 (e.g. 9876543210)", { toastId: "signUpPhone" })
      return false
    }
    setLoading(true)
    try {
      await api.post("/users/sign-up", { full_name, phone: normalized })
      return true
    } catch (err) {
  ...
  const verifySignUpOtp = async (full_name, phone, otp) => {
    ...
    setLoading(true)
    try {
      const res = await api.post("/users/verify-sign-up-otp", { full_name, phone: normalized, otp })
      setUser(res.data.user)
      navigate("/dashboard", { replace: true })
      return true
    } catch (err) {
