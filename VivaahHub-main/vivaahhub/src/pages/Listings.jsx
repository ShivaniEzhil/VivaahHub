...
const Listings = () => {
  ...
  const categoryParam = searchParams.get("category") || categoryFromPathParam(categoryFromPath);
  ...
  useEffect(() => {
    document.title = searchQuery
      ? `Search Results for "${searchQuery}" | VivaahHub`
      : categoryParam
      ? `${categoryParam} | VivaahHub`
      : "Service Listings | VivaahHub";

    const loadFilters = async () => {
      try {
        const filterData = await fetchServiceFilters();
        const cities = filterData.cities || [];
        const requiredCities = INDIAN_CITIES;
        ...
        console.error("Failed to load filters, using defaults:", err);
        setFilters({
          cities: INDIAN_CITIES,
          categories: SERVICE_CATEGORIES,
          refundPolicies: ["Non-refundable", "Partially Refundable", "Fully Refundable"],
        });
      }
    };
  ...
  useEffect(() => {
    if (cityFromUrl) {
      setActiveFilters((prev) => ({
        ...prev,
        city: prev.city.length ? prev.city : [cityFromUrl],
      }));
    }
  }, [cityFromUrl]);
          ...
          <div className="deals-main-content">
            ...
            {loading ? (
              ...
              <div className="deals-no-results">
                <p>No services found. Try adjusting your filters or search.</p>
                <p className="text-muted small mt-2">
                  New listings appear after a vendor adds a service and an admin approves it.
                  Run <code>npm run seed</code> in the backend folder for sample data.
                </p>
              </div>
            ) : (
