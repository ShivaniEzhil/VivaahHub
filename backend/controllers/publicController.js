...
export const getServiceFilters = async (req, res) => {
  try {
    ...
    const uniqueCategories = [...new Set([...categories, "Wedding Cards"])];
    const uniqueCities = [...new Set([...cities.filter((loc) => loc?.trim()), ...INDIAN_CITIES])];

    return res.status(200).json(
      formatResponse({
        categories: uniqueCategories.length ? uniqueCategories : SERVICE_CATEGORIES,
        cities: uniqueCities,
        refundPolicies: refundPolicies.filter(Boolean).length
          ? refundPolicies.filter(Boolean)
          : DEFAULT_REFUND_POLICIES,
      })
    );
  } catch (error) {
    console.error("Service filters error (using defaults):", error.message);
    return res.status(200).json(
      formatResponse({
        categories: SERVICE_CATEGORIES,
        cities: INDIAN_CITIES,
        refundPolicies: DEFAULT_REFUND_POLICIES,
      })
    );
  }
};
...
export const submitContactInquiry = async (req, res) => {
  try {
    ...
    if (!isValidIndianPhone(normalizedPhone)) {
      return res.status(400).json({ message: "Invalid phone number format. Use +91 followed by 10 digits (e.g. +919876543210)" });
    }
