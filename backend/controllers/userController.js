...
export const signIn = async (req, res) => {
  ...
  if (!isValidIndianPhone(phone)) return res.status(400).json({ message: "Invalid Indian phone number. Use +91 followed by 10 digits (e.g. +919876543210)" })
...
export const verifySignInOtp = async (req, res) => {
  ...
  if (!req.body.phone || !otp) return res.status(400).json({ message: "Phone and OTP are required" })
...
export const signUp = async (req, res) => {
  ...
  if (!full_name || !req.body.phone) return res.status(400).json({ message: "Full name and phone are required" })
  if (!isValidIndianPhone(phone)) return res.status(400).json({ message: "Invalid Indian phone number. Use +91 followed by 10 digits (e.g. +919876543210)" })
...
export const registerVendor = async (req, res) => {
  const { password, vendorRequest, brand_icon } = req.normalizedBody
  const phone = normalizeIndianPhone(req.normalizedBody.phone || "")

  if (!req.normalizedBody.phone || !password || !vendorRequest?.category) {
